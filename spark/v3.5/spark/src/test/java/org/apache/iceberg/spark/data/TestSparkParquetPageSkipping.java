...
public class TestSparkParquetPageSkipping {
  ...
  */
  ...
  public void testSinglePageMatchExpressionIn() {
    // Parquet In predicate is internally transformed into `57 <= x <= 113`.
    Expression filter = Expressions.in("_long", 57, 90, 113);
  ...
  public void testSinglePageMatchUnorderedColumnExpressionIn() {
    // Parquet In predicate is internally transformed into `zz <= x <= zzz`.
  ...
  public void testMultiplePagesMatchExpressionIn() {
    // Parquet In predicate is internally transformed into `57 <= x <= 227`.
    Expression filter = Expressions.in("_long", 57, 100, 180, 227);

    // page-1  -> row ranges: [57, 113]
    // page-2  -> row ranges: [114, 170]
  ...
  @TestTemplate
  public void testMultipleRowGroupsMatchExpressionIn() {
    // Parquet In predicate is internally transformed into `570 <= x <= 649`.
    Expression filter = Expressions.in("_long", 570, 600, 649);

    // page-10 in row group 0 -> row ranges: [570, 592]
    // page-0 in row group 1 -> row ranges: [593, 649]
    List<GenericData.Record> expected = selectRecords(allRecords, Pair.of(570, 650));
    readAndValidate(filter, expected, allRecords);
  }

  @TestTemplate
  public void testMultipleRowGroupsMatchUnorderedColumn() {
    Expression filter = Expressions.equal("_string", "zu");
    ...
    // page-22                        0                                            zv
    List<GenericData.Record> expected =
        selectRecords(allRecords, Pair.of(78, 92), Pair.of(608, 627), Pair.of(961, 981));
    readAndValidate(filter, expected, allRecords);
  }
